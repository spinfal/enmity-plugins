...
async function fetchUserBadges(id: string): Promise<string[]> {
    const res = await fetch("https://raw.githubusercontent.com/notmarek/enmitybadges/main/" + id + ".json", {
        headers: {
...
async function fetchBadge(type: string): Promise<Badge> {
    const res = await fetch("https://raw.githubusercontent.com/notmarek/enmitybadges/main/" + `data/${type}.json`, {
        headers: {
            "Cache-Control": "no-cache",
        },
    })
